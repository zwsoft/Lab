========================================================================
    活动模板库 : ATLClient 项目概述
========================================================================

应用程序向导已为您创建了此 ATLClient 项目，作为编写可执行程序 (EXE) 
的起点。
...
ATLClient.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(ATLClient.h)
        GUID 定义			(ATLClient_i.c)
        类型库				(ATLClient.tlb)
        封送处理代码			(ATLClient_p.c 和 
                                           dlldata.c）

ATLClient.h
    此文件包含 ATLClient.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 ATLClient.pch 的预编译头 (PCH) 文件和
...
ATLClientps.vcxproj
    ...
    文件。此进程生成 dlldata.c、ATLClient_i.c 和 
    ATLClient_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

ATLClientps.vcxproj.filters
